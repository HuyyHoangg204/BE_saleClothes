...
public interface FavoriteProductRepository extends JpaRepository<FavoriteProduct, Long> {
    @Query("SELECT COUNT(fp) > 0 FROM FavoriteProduct fp " + "WHERE fp.khachHang.khUserName = :username "
            + "AND fp.sanPham.product_id = :productId")
    boolean existsByUserAndProduct(@Param("username") String username, @Param("productId") int productId);
    ...
    @Modifying
    @Query(
            "delete FavoriteProduct fp where fp.khachHang.khUserName = :username and fp.sanPham.product_id = :productId ")

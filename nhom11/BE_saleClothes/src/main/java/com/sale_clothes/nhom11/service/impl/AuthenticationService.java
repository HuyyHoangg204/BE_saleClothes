...
public class AuthenticationService {
    ...
    public String generateToken(KhachHangDTO khachHangDTO) {
        ...
        JWTClaimsSet jwtClaimsSet = new JWTClaimsSet.Builder()
                ...
                .issuer("localhost") // là claim chỉ định ai đã phát hành token này, thường là tên máy chủ hoặc dịch vụ
                // phát hành
                .issueTime(
                        new Date()) // là claim ghi lại thời điểm token được phát hành, ở dây là lấy thời gian hiện tại
                .expirationTime(
        ...
        try {
            jwsObject.sign(
                    new MACSigner(SIGNER_KEY.getBytes())); // Ký JWSObject bằng cách sử dụng MACSigner và khóa bí mật
            return jwsObject
                    .serialize(); // Chuyển đổi JWSObject đã ký thành một chuỗi JWT hoàn chỉnh để có thể sử dụng.
    ...
    public AuthenticationResponse authenticate(KhachHangDTO khachHangDTO) {
        KhachHang khachHang = khachHangRepository
                .findById(KhachHangMapper.mapToKhachHang(khachHangDTO).getKhUserName())
                .orElseThrow(() -> new AppException(ErrorCode.USER_EXISTED));

        boolean authenticated = passwordEncoder.matches(
                KhachHangMapper.mapToKhachHang(khachHangDTO).getKhPassWord(), khachHang.getKhPassWord());
        if (!authenticated) throw new AppException(ErrorCode.PASSWORD_FAILED);
    ...
    public AuthenticationResponse refreshToken(RefreshRequest request) throws ParseException, JOSEException {
        ...
        var khachHang =
                khachHangRepository.findById(username).orElseThrow(() -> new AppException(ErrorCode.UNAUTHENTICATED));
